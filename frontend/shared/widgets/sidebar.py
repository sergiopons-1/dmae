...
class SidebarEspecialista(QWidget):
...
class SidebarPaciente(QWidget):
    go_perfil = pyqtSignal()
    go_ajustes = pyqtSignal()
    go_mi_progreso = pyqtSignal()
    go_logout = pyqtSignal()


    def __init__(self, nombre: str = "Paciente", parent=None, router=None):
        super().__init__(parent)
        self.nombre = nombre
        self.router = router
        if self.router is None and parent is not None and hasattr(parent, "router"):
    ...
    def _build_ui(self):
        ...
        layout.addWidget(self._nav_button("Mi perfil", self.perfil))
        layout.addWidget(self._separator())
        layout.addWidget(self._nav_button("Ajustes", self.ajustes))
        layout.addWidget(self._separator())
        layout.addWidget(self._nav_button("Mi progreso", self.mi_progreso))
        layout.addWidget(self._separator())

        spacer = QWidget()
        spacer.setStyleSheet(f"background-color: {SIDEBAR_BG};") 
        spacer.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
        layout.addWidget(spacer)
    ...
    def _build_header(self) -> QWidget:
        ...
        foto = Imagenes(enlace="assets/images/foto_paciente.png", ancho=80, alto=80)
        foto.setAlignment(Qt.AlignmentFlag.AlignCenter)
        col.addWidget(foto, alignment=Qt.AlignmentFlag.AlignHCenter)
    ...
    def set_nombre(self, nombre: str):
        self.nombre = (nombre or "Paciente").strip() or "Paciente"
        if hasattr(self, "nombre_lbl"):
            self.nombre_lbl.setText(self.nombre)
    ...
    def cerrar_sesion(self):
        if self.router is not None and hasattr(self.router, "logout_patient_session"):
    ...
    def perfil(self):
        self.go_perfil.emit()
        if self.router is not None and hasattr(self.router, "show_perfil_paciente"):
            self.router.show_perfil_paciente()

    def ajustes(self):
        self.go_ajustes.emit()
        if self.router is not None and hasattr(self.router, "show_ajustes_paciente"):
            self.router.show_ajustes_paciente()

    def mi_progreso(self):
        self.go_mi_progreso.emit()
        if self.router is not None and hasattr(self.router, "show_mi_progreso_paciente"):
