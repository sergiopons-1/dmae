...
class FinMinijuego(QWidget):
	...
	def __init__(self, parent=None):
		...
		self._resultado_superado = False
		self._puntos_finales = 0
	...
	def _construir_ui(self):
		...
		self._panel_info.setObjectName("panelInfo")
		self._panel_info.setStyleSheet(
			"QFrame#panelInfo { background-color: #FFFFFF; border: 2px solid #0E4C66; }"
		)
		...
		self._txt_info.lbl.setStyleSheet("color: #0E4C66; background-color: #FFFFFF; border: none;")
	...
	def set_resultado(self, libros_colocados: int):
		...
		superado = puntos >= 2
		self._resultado_superado = superado
		self._puntos_finales = puntos
	...
	def ha_superado(self) -> bool:
		return self._resultado_superado

	def puntos_finales(self) -> int:
		return self._puntos_finales
